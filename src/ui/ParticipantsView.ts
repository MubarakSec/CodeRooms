...
class ParticipantItem extends vscode.TreeItem {
  ...
  constructor(participant: Participant, isSelf: boolean, canManage: boolean, isTyping: boolean, currentFile?: string) {
    ...
    const descriptionParts: string[] = [roleBadge(participant.role)];
    if (modeDetail) {
...
export class ParticipantsView implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  private createSessionHeader(): vscode.TreeItem {
    ...
    const header = new BlockItem(
      Block.Session,
      roomId ? `Room ${roomId}` : 'CodeRooms',
      descriptionParts.join(' · ') || undefined,
      icon
    );
  ...
  private countRoles(): { root: number; collab: number; viewer: number } {
    return this.roomState.getParticipants().reduce(
      (acc, p) => {
        if (p.role === 'root') acc.root += 1;
        else if (p.role === 'collaborator') acc.collab += 1;
        else if (p.role === 'viewer') acc.viewer += 1;
        return acc;
      },
